...
class SpatialCrossAttention(nn.Module):
    """Spatial cross-attention operating on 3D feature maps"""
    
    def __init__(self, channels=256, num_heads=8, dropout=0.1):
        super().__init__()
        self.channels = channels
        
        # Multi-head attention over spatial positions
        self.multihead_attn = nn.MultiheadAttention(
            embed_dim=channels,
            num_heads=num_heads,
            dropout=dropout,
...
class ESP_CNN_Encoder(nn.Module):
    ...
    def __init__(self, input_channels=8, embedding_dim=256, dropout=0.3, return_spatial=False):
        ...
        # Conservative downsampling: only ONE stride=2 layer
        # Keep spatial resolution high for better 3D interaction modeling
        self.block1 = ResidualBlock3D(32, 64, stride=2, dropout=dropout, dilation=1)   # Downsample once
        self.block2 = ResidualBlock3D(64, 128, stride=1, dropout=dropout, dilation=1)  # No dilation
        self.block3 = ResidualBlock3D(128, 192, stride=1, dropout=dropout, dilation=1) # No dilation  
        self.block4 = ResidualBlock3D(192, 256, stride=1, dropout=dropout, dilation=2) # Light dilation only
        self.block5 = ResidualBlock3D(256, 256, stride=1, dropout=dropout, dilation=1) # Final standard block
    ...
    def forward(self, x):
        ...
        x = self.block1(x)               # [B, 64, H/2, W/2, D/2] - only downsampling
        x = self.block2(x)               # [B, 128, H/2, W/2, D/2]
        x = self.block3(x)               # [B, 192, H/2, W/2, D/2]
        x = self.block4(x)               # [B, 256, H/2, W/2, D/2] - light dilation=2
        x = self.block5(x)               # [B, 256, H/2, W/2, D/2]
...
class ESP_JointNet(nn.Module):
    ...
    def __init__(self, embedding_dim=256, temperature=0.07, dropout=0.3, 
        ...
        if use_pairwise_attention:
            ...
            # Downsample spatial maps before attention to reduce memory
            # Ligand: 12³ → 6³ (216 tokens)
            # Pocket: 24³ → 12³ (1,728 tokens)
            # Attention: 216 × 1,728 = 373K token pairs (~64x reduction from 23.9M)
            self.pool_before_attn = nn.AvgPool3d(kernel_size=2, stride=2)
            
            # Spatial cross-attention over downsampled 3D feature maps
            self.spatial_cross_attn_p = SpatialCrossAttention(channels=256, num_heads=num_heads, dropout=0.1)
            self.spatial_cross_attn_l = SpatialCrossAttention(channels=256, num_heads=num_heads, dropout=0.1)
    ...
    def forward(self, pocket_esp, ligand_esp):
        ...
        if self.use_pairwise_attention:
            ...
            # Downsample before attention to reduce memory
            # Pocket: 24³ → 12³ (13,824 → 1,728 tokens)
            # Ligand: 12³ → 6³ (1,728 → 216 tokens)
            feat_pocket_ds = self.pool_before_attn(feat_pocket)  # [B, 256, 12, 12, 12]
            feat_ligand_ds = self.pool_before_attn(feat_ligand)  # [B, 256, 6, 6, 6]
            
            # Spatial cross-attention (residuals handled internally via norm1/norm2)
            feat_p_ctx = self.spatial_cross_attn_p(feat_pocket_ds, feat_ligand_ds)  # [B, 256, 12, 12, 12]
            feat_l_ctx = self.spatial_cross_attn_l(feat_ligand_ds, feat_pocket_ds)  # [B, 256, 6, 6, 6]
...
if __name__ == '__main__':
    ...
    model = ESP_JointNet(embedding_dim=512, temperature=0.07)
    
    # Create dummy inputs
    batch_size = 4
    pocket_esp = torch.randn(batch_size, 1, 64, 64, 64)
    ligand_esp = torch.randn(batch_size, 1, 32, 32, 32)
    
    # Forward pass
    z_pocket, z_ligand = model(pocket_esp, ligand_esp)
    print(f"Pocket embeddings shape: {z_pocket.shape}")
    print(f"Ligand embeddings shape: {z_ligand.shape}")
    
    # Compute loss
    loss = model.contrastive_loss(z_pocket, z_ligand)
    print(f"Contrastive loss: {loss.item():.4f}")
    
    # Test binding affinity predictor
    print("\nTesting binding affinity predictor...")
    predictor = ESP_BindingAffinityPredictor(model, freeze_encoders=True)
    affinity = predictor(pocket_esp, ligand_esp)
    print(f"Predicted affinity shape: {affinity.shape}")
